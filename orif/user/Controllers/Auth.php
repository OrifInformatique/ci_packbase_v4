...
class Auth extends BaseController {
    ...
    private function create_azure_begin_request(string $ad_tenant,
        string $graphUserScopes, string $client_id,
        string $redirect_uri): String
    {
        $url = "https://login.microsoftonline.com/" . $ad_tenant 
    ...
    public function azure_login_begin(?array $azureData = null): Response
    {
        // Authentication part begins
        $client_id = $azureData['CLIENT_ID'] ?? getenv('CLIENT_ID');
        $ad_tenant = $azureData['TENANT_ID'] ?? getenv('TENANT_ID');
        $graphUserScopes = $azureData['GRAPH_USER_SCOPES'] 
            ?? getenv('GRAPH_USER_SCOPES');
        $redirect_uri = $azureData['REDIRECT_URI'] ?? getenv('REDIRECT_URI');
        $url = $this->create_azure_begin_request($ad_tenant, $graphUserScopes,
    ...
    private function get_options_azure_token(string $client_id,
        string $redirect_uri, string $client_secret): array
    {
    ...
    private function after_azure_login(array $userdata): string|Response
    {
        ...
        $ci_user_azure = $this->user_model
                              ->where('azure_mail', $user_azure_mail)->first();

        // Seperate name and lastname from email for mail correspondances
        // True = before '@' and without '@'
        $nameAndLastname = strstr($user_azure_mail, '@', true); 
        // Azure mail found
        if (!empty($ci_user_azure)) {
            $_SESSION['user_id'] = $ci_user_azure['id'];
            $_SESSION['username'] = $ci_user_azure['username'];
            $_SESSION['user_access'] = (int)$this->user_model
                                         ->get_access_level($ci_user_azure);
            return redirect()->to($_SESSION['after_login_redirect']);
        }
        ...
        $_SESSION['user_access'] = config("\User\Config\UserConfig")
            ->azure_default_access_lvl;
        $_SESSION['azure_mail'] = $user_azure_mail;
        $correspondingUser = $this->user_model
                      ->where('email LIKE', $nameAndLastname . '%')->first();
        $correspondingEmail = !isset($correspondingUser) ? '' 
            : $correspondingUser['email'];
        $output = array(
    ...
    private function get_azure_user_data($token): string|false
    {
        ...
        $json = file_get_contents("https://graph.microsoft.com/v1.0/me", false,
            $context);
        return $json;
    }
     ...
     * @return void
     */
    public function azure_login(?array $azureData = null): string|Response
    {
        $client_id = $azureData['CLIENT_ID'] ?? getenv('CLIENT_ID');
        $ad_tenant = $azureData['TENANT_ID'] ?? getenv('TENANT_ID');
        $redirect_uri = $azureData['REDIRECT_URI'] ?? getenv('REDIRECT_URI');
        ...
        $options = $this->get_options_azure_token($client_id, $redirect_uri,
            $client_secret);
        $context  = stream_context_create($options);
        // Special error handler to verify if "client secret" is still valid
        try {
            $json = file_get_contents("https://login.microsoftonline.com/"
                . $ad_tenant . "/oauth2/v2.0/token", false, $context);
        } catch (\Exception $e) {
            $data['title'] = 'Azure error';
            $data['Exception'] = $e;
        ...
        $json = $this->get_azure_user_data($authdata["access_token"]);
        if ($json === false) {
        ...
        return $this->after_azure_login($userdata);
    }
