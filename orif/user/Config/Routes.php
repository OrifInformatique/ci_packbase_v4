...
$routes->group("user/admin",function($routes){
   ...
   $routes->add("delete_user/(:num)","\User\Controllers\Admin::delete_user/$1",['filter'=>"logfilter:{config('\User\Config\User_config')->access_lvl_admin}"]);
   $routes->add("delete_user/(:num)/(:num)","\User\Controllers\Admin::delete_user/$1/$2",['filter'=>"logfilter:{config('\User\Config\User_config')->access_lvl_admin}"]);
   $routes->add("reactivate_user/(:num)","\User\Controllers\Admin::reactivate_user/$1",['filter'=>"logfilter:{config('\User\Config\User_config')->access_lvl_admin}"]);
   $routes->add("password_change_user/(:num)","\User\Controllers\Admin::password_change_user/$1",['filter'=>"logfilter:{config('\User\Config\User_config')->access_lvl_admin}"]);
